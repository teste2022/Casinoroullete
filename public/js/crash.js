$(function() {
    ...
    window.crash = {
        ...
        _options: {
            ...
            yaxis: {
                ...
                tickFormatter: function(val) {
                    if (val==1) return '';
                    return '<span style="color: white;">'+val.toFixed(1)+'x</span>';
                },
				tickLength: 10,
				minTickSize: 0.1,
				ticks: function(axis) {
					var tmp = axis.tickGenerator(axis);
						if(tmp[0] == 1){
							tmp[0] = 0;
					}
					return tmp;
        ...
        init: function () {
            this._options.xaxis.max = Math.max(this._i, ((Math.log(2) / Math.log(Math.E)))/this._pattern / this._pointsPerSec);
            this._options.yaxis.max = Math.max(this._multiplier, 2);
        ...
        draw: function () {
            $.plot(this._chart, [ {data: this._data, lines:  {fill: false}} ], this._options);
        },
        resize: function () {
            this.draw();
        },
        ...
        crash_start: function (data) {
            ...
            this._dateStart = data.time;
            this._chartInfo
                .removeClass("crash-info")
                .addClass("play-info");
            this.crash_draw(data.multiplier);
        },
        crash_tick: function (data) {
            this.status = 'game';
			var localMultiplier = this._multiplier;
            cancelAnimationFrame(this.interval);
			
            this.interval = requestAnimationFrame(function () { crash.crash_draw(data.multiplier); });
			
			var diff = parseFloat(localMultiplier - data.multiplier).toFixed(2);
            if (diff > 0.01 || diff < -0.01) {
			//	console.log('Local multiplier desynchronized! Local: ' + localMultiplier + ', remote: ' + data.multiplier + ', diff: ' + parseFloat(localMultiplier - data.multiplier).toFixed(2)); 
				this._dateStart = Date.now() - (Math.log(data.multiplier) / (Math.log(Math.E) * this._pattern));
			}
			if(this._myBet){
				$('.bet-butt').text((data.multiplier * this._myBet).toFixed(0));
			}


        },
        crash_draw: function (multiplier) {
            while(this._multiplier < multiplier) {
                this.crash_add();
            }
            this._options.xaxis.max = Math.max(this._i, ((Math.log(2) / Math.log(Math.E)))/this._pattern / this._pointsPerSec);
            this._options.yaxis.max = Math.max(this._multiplier, 2);
            this._chartInfo.text(this._multiplier.toFixed(2)+'x');
        ...
        crash_add: function() {
            this._i++;
            this._multiplier = Math.pow(Math.E, this._pattern * this._i * this._pointsPerSec);
            this._data.push([ this._i, this._multiplier ]);
        },
        ...
        player_bet: function (data) {
            $('<tr class="player-bet' + (data.multiplier ? ' win' : '') + (data.profile.steamid == steamid ? ' my' : '') + '" data-steamid="'+data.profile.steamid+'" data-bet="'+data.bet+'">' +
                ...
                '<td>'+data.bet+'</td>' +
                '<td class="' + ((data.multiplier || 0) < 5 ? 'text-white' : (data.multiplier < 30 ? 'text-green' : (data.multiplier < 300 ? 'text-blue' : 'text-gold'))) + '">' + (data.multiplier ? parseFloat(data.multiplier).toFixed(2) : '?') + '</td>' +
        ...
        history: function (game) {
            ...
            $(".box-history table tbody").prepend('<tr class="'+bet_class+'"><td class="multiplier '+(game.multiplier < 2 ? 'text-red' : (game.multiplier < 5 ? '' : (game.multiplier < 30 ? 'text-green' : (game.multiplier < 300 ? 'text-blue' : 'text-gold'))))+'">' + parseFloat(game.multiplier).toFixed(2) + 'x</td><td class="date">' + time + '</td><td>' + game.hash + '</td><td>' + game.secret + '</td></tr>');
        }
    };
    crash.init();
